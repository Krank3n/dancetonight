...
export const DanceStyleIcon: React.FC<{ style: DanceStyle, className?: string }> = ({ style, className = "w-8 h-8 text-pink-500" }) => {
  // Generic dance icon, can be expanded with specific icons per style
  switch(style) {
    case DanceStyle.Salsa:
      return <span className={`font-bold text-sm ${className}`}>SA</span>; // Placeholder text icon
    case DanceStyle.Bachata:
      return <span className={`font-bold text-sm ${className}`}>BA</span>;
    case DanceStyle.Tango:
      return <span className={`font-bold text-sm ${className}`}>TA</span>;
    default:
      return (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
